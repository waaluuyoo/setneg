...
foreach($edit->result_array() as $row){
	$id = $row['slide_id'];
	$judul = $row['slide_judul'];
	$isi = $row['slide_isi'];
	$gambar = $row['slide_gambar'];
	$tact = 'Edit';
	$act = 'edit_slide';
}
...
<div class="page">
	<div class="page-header">
      ...
      <div class="page-header-actions">
		<a type="button" class="btn btn-warning" href="<?=base_url()?>front/slide">
			...
			<div class="col-lg-12">
			  ...
			  <div class="panel">
				<div class="panel-heading">
				  <h3 class="panel-title"><?=$tact?> <?=$title?></h3>
				</div>
				<div class="panel-body">
				  <form class="form-horizontal" id="FrmAjax" enctype="multipart/form-data">
				  <input type="hidden" name="id" value="<?=$id?>"/>
					...
					<div class="form-group row">
					  ...
					  <div class="col-md-6">
						...
						<input type="hidden" class="form-control" name="gambar" value="<?=$gambar?>"/><br><br>
						<?=($gambar != '' ? '<img src="'.base_url().'/assets/media/slide/'.$gambar.'" width="300">' : '')?>
					  </div>
					</div>
			...
			$("#FrmAjax").on('submit',(function(e) {
				...
				success: function(data)   // A function to be called if request succeeds
				{
					if(data == '1'){
						...
						location.href="<?=base_url()?>front/slide"
					}else{
						  $('.alert').show();
