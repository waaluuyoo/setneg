...
class CI_Accesslevel {
	
	var $elementArray = array();
	
	
	
	function addToArray($id,$name,$parentID,$level,$type,$enabled,$edit,$delete){
		if(empty($parentID))$parentID=0;	
		$this->elementArray[$parentID][] = array($id,$name,$parentID,$level,$type,$enabled,$edit,$delete);
	}
	...
	function drawTree(){
		...
		for($no=0;$no<count($this->elementArray[0]);$no++){
			...
			echo "<td align='center'><input type='checkbox' value='Y' name='view_access_".$this->elementArray[0][$no][0]."' class='check_all_".$this->elementArray[0][$no][0]."' onclick='select_all_".$this->elementArray[0][$no][0]."()'></td>";	
		...
		if(isset($this->elementArray[$parentID])){
			...
			for($no=0;$no<count($this->elementArray[$parentID]);$no++){
				...
				echo "<td>".$tab." ".$this->elementArray[$parentID][$no][1]."<input type='hidden' name='menu[]' value='".$this->elementArray[$parentID][$no][0]."'></td>";
					echo "<td align='center'><input type='checkbox' name='view_access_".$this->elementArray[$parentID][$no][0]."' value='Y' class='view_".$this->elementArray[$parentID][$no][2]."'></td>";	
				if($this->elementArray[$parentID][$no][4] == 'B'){	
					echo "<td align='center'><input type='checkbox' name='add_access_".$this->elementArray[$parentID][$no][0]."' value='Y' class='add_".$this->elementArray[$parentID][$no][2]."'></td>";	
					echo "<td align='center'><input type='checkbox' name='edit_access_".$this->elementArray[$parentID][$no][0]."' value='Y' class='edit_".$this->elementArray[$parentID][$no][2]."'></td>";	
					echo "<td align='center'><input type='checkbox' name='delete_access_".$this->elementArray[$parentID][$no][0]."' value='Y' class='delete_".$this->elementArray[$parentID][$no][2]."'></td>";	
				}else{	
					echo "<td align='center' colspan='3'>&nbsp;</td>";		
				}
				echo "</tr>";	
